...
当前仓库包含文档、项目记忆检查脚本和第一份 Context/Canon 纵向切片。SQLite schema、FTS5 检索、Policy Gate、Context Packet、Canon transaction、fixture 和测试已实现；模型调用、编辑器、发布适配、云服务和完整产品仍未实现。
...
T002 已形成 [核心契约与 S1–S6 纸面演练](../architecture/CORE_CONTRACTS.md)。T003 已实现本地优先、模块化单体、SQLite + FTS5 和 TypeScript 应用服务；运行证据见 [纵向切片](VERTICAL_SLICE_T003.md)。可替换 Model Adapter、内容寻址大文本和 Tauri 2 + React 桌面界面是下一阶段边界。
