...
T005 已在百万字合成长篇上证明精确 FTS、隔离和延迟基线达标，同时测得无共享词同义查询 0/5。T006 已实现 ADR-013 的 adapter、cache 和 RRF，并用 fixture provider 验证语义回归与零泄漏；具体生产 embedding 模型仍需许可证与真实语料评估。图数据库、多 Agent、分布式服务和云同步只有在评估显示必要时引入。详细结果见 [T005](../quality/LONG_CONTEXT_BENCHMARK_T005.md) 与 [T006](../quality/HYBRID_RETRIEVAL_BENCHMARK_T006.md)。
